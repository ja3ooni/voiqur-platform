...
class MistralModelManager:
    ...
    async def load_model(
        self, model_type: ModelType, model_path: Optional[str] = None
    ) -> bool:
        ...
        try:
            ...
            self.models_cache[model_type] = {"model": model, "tokenizer": tokenizer}
    ...
    async def generate_response(
        ...
        """Generate response using current model or cloud API"""
    ...
    async def _generate_mock_response(
        ...
        """Mock response - raises error instead of returning mock data per LLM-02"""
        raise RuntimeError(
            "No LLM model loaded and no MISTRAL_API_KEY available. "
            "Please provide MISTRAL_API_KEY environment variable or load a local model."
        )
    ...
    async def _generate_cloud_response(
        ...
        """Generate response using Mistral Cloud API"""
        from mistralai.client import Mistral
        
        if not self._cloud_client:
            self._cloud_client = Mistral(api_key=self._mistral_api_key)
        ...
        response = await asyncio.to_thread(
            self._cloud_client.chat.complete,
            **chat_params
        )
...
class IntentRecognizer:
    ...
    async def recognize_intent(
        ...
        try:
            ...
            return IntentRecognitionResult(
                intent="general", confidence=0.0, entities={}, context_needed=[]
            )
...
class SessionManager:
    ...
    def create_session(
        self, user_id: Optional[str] = None, language: str = "en"
    ) -> ConversationContext:
...
Be culturally sensitive and adapt your responses to regional contexts.""",
    ...
    async def process_message(
        ...
        try:
            ...
            if tool_calls:
                ...
                for tool_call in tool_calls:
                    ...
                    if result.status.value == "completed":
                        context.add_message(
                            "tool",
                            f"Tool {tool_call.tool_name} result: {result.result}",
                        )
                    else:
                        context.add_message(
                            "tool",
                            f"Tool {tool_call.tool_name} failed: {result.error_message}",
                        )
            ...
            response = await self.model_manager.generate_response(
                context,
                system_prompt,
                self.tool_registry.get_openai_functions()
                if self.model_manager.supports_tools()
                else None,
            )
            ...
            self._update_performance_metrics(
                processing_time, intent_result.confidence, tool_results
            )
    ...
    async def _determine_tool_calls(
        self, intent_result: IntentRecognitionResult, context: ConversationContext
    ) -> List[ToolCall]:
        ...
        if intent_result.intent in ["question", "information"] or any(
            word in context.conversation_history[-1]["content"].lower()
        ...
        if intent_result.intent in ["support", "complaint"] or any(
            word in context.conversation_history[-1]["content"].lower()
            ...
            tool_calls.append(
                ToolCall(
                    ...
                    parameters={"input_data": last_message, "input_type": "text"},
                    caller_agent_id=self.agent_id,
                )
            )

        # Arabic language processing
        if context.language in ["ar", "arabic"] or self._contains_arabic_text(
            context.conversation_history[-1]["content"]
            if context.conversation_history
            else ""
        ):
        ...
        last_message_lower = (
            context.conversation_history[-1]["content"].lower()
            if context.conversation_history
            else ""
        )
        if any(
            word in last_message_lower
            for word in ["voice", "accent", "pronunciation", "speak", "audio"]
        ):
    ...
    def _contains_arabic_text(self, text: str) -> bool:
        ...
        try:
            ...
            self.logger.warning(f"Arabic detection failed: {e}")
            return False
    ...
    def _update_performance_metrics(
        ...
        self.performance_metrics["intent_recognition_accuracy"] = (
            current_intent_avg * (total_messages - 1) + intent_confidence
        ) / total_messages
        ...
        self.state.performance_metrics.update(
            {
                ...
                "intent_accuracy": self.performance_metrics[
                    "intent_recognition_accuracy"
                ],
                "tool_success_rate": self.performance_metrics.get(
                    "tool_success_rate", 0.0
                ),
        ...
        try:
            if message.message_type == "dialog_request":
                ...
                context = (
                    ...
                    if session_id
                    else ConversationContext(session_id="temp")
                )
                ...
                if "llm_agent" in required_agents or self.agent_id in required_agents:
                    ...
                    return AgentMessage(
                        ...
                        payload={
                            "status": "accepted",
                            "capabilities": [
                                cap.dict() for cap in self.state.capabilities
                            ],
    ...
    async def register_agent_tool(
        self, agent_id: str, tool_definition: Dict[str, Any]
    ) -> bool:
        ...
        try:
            ...
            if success:
                self.logger.info(
                    f"Registered tool {tool_definition['name']} from agent {agent_id}"
                )
    ...
    async def create_tool_chain(
        ...
        try:
            ...
            for tool_name in tools:
                ...
                if result.status.value == "completed" and isinstance(
                    result.result, dict
                ):
        ...
        return {
            ...
            "agent_integrations": len(
                [
                    ...
                    if self.tool_registry.get_tool_definition(tool).tool_type
                    == ToolType.AGENT_INTEGRATION
                ]
            ),
        }
    ...
    async def handle_interruption(
        self, session_id: str, new_input: str
    ) -> Dict[str, Any]:
