...
class AuthManager:
    ...
    def create_refresh_token(self, user: User) -> str:
        ...
        payload = {"user_id": user.id, "type": "refresh", "exp": expire, "iat": now}
    ...
    async def verify_token(self, token: str) -> TokenData:
        ...
        try:
            ...
            if self.config.require_eu_residency and not payload.get(
                "eu_resident", False
            ):
                raise HTTPException(
    ...
    async def authenticate_user(self, username: str, password: str) -> Optional[User]:
        ...
        Raises:
            ValueError: If demo mode is improperly configured
        """
        import os
        ...
        demo_email = os.getenv(
            "DEMO_EMAIL", f"demo@{os.getenv('APP_DOMAIN', 'euvoice.ai')}"
        )
        ...
        if demo_enabled and username == demo_username and password == demo_password:
            if not demo_password:
                raise ValueError("DEMO_PASSWORD must be set when DEMO_MODE is enabled")
            return User(
                id="demo-user-id",
                email=demo_email,
        ...
        if username == demo_username and demo_enabled:
            logger.warning(f"Demo login failed for username: {username}")
            return None
    ...
    async def create_oauth2_authorization_url(
        self, client_id: str, redirect_uri: str, scopes: List[str], state: str
    ) -> str:
        """
